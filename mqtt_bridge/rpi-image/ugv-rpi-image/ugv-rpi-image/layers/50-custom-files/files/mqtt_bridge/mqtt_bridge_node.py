...
try:
    from .cyberwave_mqtt_adapter import CyberwaveAdapter, SOURCE_TYPE_EDGE, SOURCE_TYPE_TELE, SOURCE_TYPE_EDIT, SOURCE_TYPE_SIM
except Exception:
...
class MQTTBridgeNode(Node):
    def __init__(self) -> None:
        ...
        # Log Cyberwave environment variables for debugging
        ...
        self.declare_parameter('cyberwave.api_token', '')
        self.declare_parameter('topic_prefix', '')

        # Get values from parameters (which are loaded from params.yaml)
        p_token = self.get_parameter('broker.cyberwave_token').value or self.get_parameter('cyberwave.api_token').value
        ...
        token = p_token or os.getenv('CYBERWAVE_TOKEN')
        ...
        # 'bridge' is a potentially complex mapping. Declaring a dict as a
        # default is rejected by rclpy, so try to read a provided 'bridge'
        # parameter first (if the launch/params system provided it). If not
        # available, fall back to loading the package's config/params.yaml
        # file (useful during local testing).
        bridge = {}
        try:
            # attempt to read a user-provided 'bridge' param (may raise if not declared)
            bridge_val = self.get_parameter('bridge').value
            if isinstance(bridge_val, dict):
                bridge = bridge_val
        except Exception:
            # fallback: load config/params.yaml from package share if present
            try:
                ...
                if os.path.exists(cfg_path):
                    ...
                    # params.yaml often nests under patterns like '/*/*: ros__parameters'
                    def find_ros_parameters(d):
                        if isinstance(d, dict):
                            if 'ros__parameters' in d:
                                return d['ros__parameters']
                            for v in d.values():
                                res = find_ros_parameters(v)
                                if res:
                                    return res
                        return None

                    ros_params = find_ros_parameters(raw) or {}
        ...
        # Camera watchdog timer (every 5 seconds)
        ...
        if self._mapping is not None and hasattr(self._mapping, 'twin_uuid') and self._mapping.twin_uuid:
            ...
            self.get_logger().warning(
                "No twin_uuid available in mapping - tool0 joint control will not be available. "
                "Ensure your mapping file contains a valid 'twin_uuid' in metadata."
            )
    ...
    def _load_mapping(self) -> None:
        ...
        try:
            ...
            if not uuid and require_uuid:
                raise ValueError(f"Mapping {path} missing 'twin_uuid'")
        except Exception as e:
            self._mapping = None
            raise
    ...
    def _make_ros_cb(self, ros_topic: str, mqtt_topic: str, msg_cls: typing.Any) -> typing.Callable[[typing.Any], None]:
        ...
        def cb(msg):
            try:
                ...
                elif (msg_cls is Float32 or msg_cls.__name__ == 'Float32') and '/status/battery' in mqtt_topic:
                    self._last_battery_msg = msg
    ...
    def _encode_msg_for_mqtt(self, msg, msg_cls, mqtt_topic: str = None) -> typing.Union[str, bytes]:
        ...
        try:
            ...
            if (msg_cls is Float32 or msg_cls.__name__ == 'Float32') and mqtt_topic and '/status/battery' in mqtt_topic:
                try:
    ...
    def _handle_mqtt_message(self, topic: str, payload=None, mqtt_msg=None) -> None:
        ...
        # COMMAND ROUTER: Handle cyberwave/twin/{twin_uuid}/command messages
                ...
                with open(path, 'r') as f:
                    for line in f:
                        ...
                        if '=' in line:
                            ...
                            os.environ[key.strip()] = val.split('#', 1)[0].strip().strip('"').strip("'")
                return True
        return False
