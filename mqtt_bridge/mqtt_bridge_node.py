...
from std_msgs.msg import String, Int32, Float32, UInt32MultiArray, Float32MultiArray, Float64MultiArray
...
class MQTTBridgeNode(Node):
    def __init__(self) -> None:
        ...
        mqtt_broker_env = os.getenv('CYBERWAVE_MQTT_HOST') or os.getenv('CYBERWAVE_MQTT_BROKER')
        ...
        token = p_token or os.getenv('CYBERWAVE_API_KEY') or os.getenv('CYBERWAVE_TOKEN')
        ...
        sdk_handler.setFormatter(logging.Formatter('[%(name)s] %(levelname)s: %(message)s'))
        ...
        if debug_logs_enabled:
            self.get_logger().info("Debug logs ENABLED - showing aiortc, aioice, google, asyncio, cyberwave debug messages")
            for lib in ["aiortc", "aioice", "google", "asyncio", "cyberwave"]:
                logging.getLogger(lib).setLevel(logging.DEBUG)
        else:
            self.get_logger().info("Debug logs disabled - aiortc, aioice, google, asyncio set to WARNING level (cyberwave at INFO)")
        ...
        default_rate_hz = float(rate_limit_env) if rate_limit_env else DEFAULT_UPSTREAM_PUBLISH_RATE_HZ
        ...
        self._disable_edge_joint_updates = self.get_parameter('disable_edge_joint_updates').value
        ...
        try:
            ...
            if pkg_share:
                ...
                if os.path.exists(cfg_path):
                    ...
                    if '/mqtt_bridge_node' in raw and 'ros__parameters' in raw['/mqtt_bridge_node']:
                        ros_params = raw['/mqtt_bridge_node']['ros__parameters']
                        bridge = ros_params.get('bridge', {}) or {}
                        
                        ros2mqtt_topics = bridge.get('ros2mqtt', {}).get('ros_topics', [])
                        mqtt2ros_topics = bridge.get('mqtt2ros', {}).get('mqtt_topics', [])
                        ...
                        self.get_logger().warning(f"No ros__parameters found in {cfg_path}")
                else:
                    self.get_logger().error(f"Config file not found: {cfg_path}")
            else:
                self.get_logger().error("Could not find mqtt_bridge package share directory")
        except Exception as e:
            import traceback
            self.get_logger().error(f"Failed to load bridge config: {e}\n{traceback.format_exc()}")
            bridge = {}
        ...
        try:
            self._ros2mqtt_rate_hz = float(self.get_parameter('ros2mqtt_rate_limit').value)
            ...
            self._ros2mqtt_rate_hz = DEFAULT_UPSTREAM_PUBLISH_RATE_HZ  # default fallback
        ...
        def _run_async_loop(loop):
            ...
            try:
                ...
                pass
        self._async_loop_thread = threading.Thread(target=_run_async_loop, args=(self._async_loop,), daemon=True)
        self._async_loop_thread.start()

        # Initialize health publisher
        self._health_publisher = HealthPublisher(self)
        self._health_timer = self.create_timer(5.0, self._health_publisher.publish_health_status)

        # Camera watchdog - only enable if usb_cam package is available
        self._last_camera_check_time = 0
        self._last_image_time = time.time() # Initialize with current time for grace period
        ...
        try:
            ...
            self._camera_watchdog_timer = self.create_timer(5.0, self._check_camera_status)
            self.get_logger().info("Camera watchdog enabled (usb_cam package found)")
        except Exception:
            self._usb_cam_available = False
            self.get_logger().info("Camera watchdog disabled (usb_cam package not installed)")
        ...
        self.get_logger().debug(f"Broker configuration: host={host}, port={port}, user={username}, use_cyberwave={use_cw}")
        ...
        if use_cw and CyberwaveAdapter is not None:
            ...
            if token:
                try:
                    ...
                    self.get_logger().info("✓ Cyberwave SDK adapter initialized successfully")
                    self.get_logger().info("  Connection method: Cyberwave SDK (PRIMARY)")
                    self.get_logger().info("=" * 60)
                except Exception as e:
                    self._mqtt_adapter = None
                    self.get_logger().error(f"Failed to initialize Cyberwave adapter with token: {e}")
                    self.get_logger().warning("Will fall back to paho-mqtt client")
            else:
                self.get_logger().error("Cyberwave API Token not found! 'use_cyberwave' is enabled but no token was provided via parameters or environment (CYBERWAVE_TOKEN). Video streaming will be unavailable.")
        ...
        if username and password:
            ...
            self.get_logger().info(f"MQTT authentication configured for user: {username}")
        ...
        try:
            ...
            if self._mapping and getattr(self._mapping, 'twin_uuid', None):
                ...
                self.get_logger().info(f"ROBOT_ID: {self.get_parameter('robot_id').value}")
        ...
        if self._mapping is not None and self._mapping.command_registry:
            try:
                ...
                if self._mapping is not None and getattr(self._mapping, 'twin_uuid', None):
                    cmd_topic = cmd_topic_template.replace('{twin_uuid}', self._mapping.twin_uuid)
                ...
                self._command_registry.set_mqtt_context(self._mqtt_adapter or self._mqtt_client, cmd_topic)
                ...
                self.get_logger().warning(f"Could not initialize command registry from {self._mapping.command_registry}: {e}")
        else:
        ...
        try:
            ...
            self._reload_srv = self.create_service(Trigger, fq_reload_name, self._on_reload_mapping)

            # Start Camera Service
            fq_start_video_name = f"{ns}/{self.get_name()}/start_video"
            self._start_video_srv = self.create_service(Trigger, fq_start_video_name, self._on_start_video)

            # Stop Camera Service
            fq_stop_video_name = f"{ns}/{self.get_name()}/stop_video"
            self._stop_video_srv = self.create_service(Trigger, fq_stop_video_name, self._on_stop_video)
            
            self.get_logger().info(f"WebRTC Video Services initialized: {fq_start_video_name}, {fq_stop_video_name}")
        ...
        try:
            ...
            auto_start_delay = float(self.get_parameter('webrtc.auto_start_delay_sec').value)
            auto_start_retry = float(self.get_parameter('webrtc.auto_start_retry_sec').value)
        except Exception:
            auto_start = False
        ...
        if auto_start:
            ...
            self.get_logger().info(f"WebRTC auto-start enabled, scheduling start in {delay:.1f}s")
            self._auto_start_timer = self.create_timer(delay, self._maybe_auto_start_webrtc)
        ...
        try:
            mapping_reload_on_change = bool(self.get_parameter('mapping_reload_on_change').value)
        except Exception:
        ...
        if mapping_reload_on_change and getattr(self, '_mapping', None) is not None:
            try:
                ...
                self._mapping_watcher_thread = threading.Thread(target=self._mapping_watcher_loop, daemon=True)
        ...
        for ros_topic in ros_topics:
            ...
            if '{twin_uuid}' in mqtt_topic and self._mapping is not None and hasattr(self._mapping, 'twin_uuid') and self._mapping.twin_uuid:
                mqtt_topic = mqtt_topic.replace('{twin_uuid}', self._mapping.twin_uuid)
            
            if hasattr(self, 'ros_prefix') and self.ros_prefix and not mqtt_topic.startswith(self.ros_prefix):
            ...
            sub = self.create_subscription(msg_cls, ros_topic, self._make_ros_cb(ros_topic, mqtt_topic, msg_cls), qos)
            ...
            sdk_method = mapping.get('sdk_method') if isinstance(mapping, dict) else None
            if sdk_method:
        ...
        topics_to_subscribe = list(mqtt_topics) + [t for t in per_topic_keys if t not in mqtt_topics]
        for mqtt_topic_template in topics_to_subscribe:
            ...
            if not mapping:
                mapping = (mqtt2ros.get('topics', {}) or {}).get(mqtt_topic_template, {}) or {}
            ...
            if isinstance(mapping, list):
                ...
                self.get_logger().info(f"Processing multi-target MQTT->ROS bridge: {mqtt_topic} -> {len(mapping)} targets")
                for map_entry in mapping:
                    if not isinstance(map_entry, dict):
                        continue
                    ros_topic = map_entry.get('ros_topic', mqtt_topic if mqtt_topic.startswith('/') else f"/{mqtt_topic}")
                    ...
                    rt_name = ros_topic if ros_topic.startswith('/') else f"/{ros_topic}"
                ...
                ros_topic = mapping.get('ros_topic', mqtt_topic if mqtt_topic.startswith('/') else f"/{mqtt_topic}")
                type_str = mapping.get('type')
                msg_cls = self._resolve_msg_class(type_str)
                ...
                if isinstance(ros_topic, list):
                    self.get_logger().info(f"Creating MQTT->ROS bridge: {mqtt_topic} -> {ros_topic} ({msg_cls.__name__})")
                    for rt in ros_topic:
                    ...
                    rt_name = ros_topic if ros_topic.startswith('/') else f"/{ros_topic}"
                    self.get_logger().info(f"Creating MQTT->ROS bridge: {mqtt_topic} -> {rt_name} ({msg_cls.__name__})")
                    existing = self._ros_topic_msgtype.get(rt_name)
                    if existing is not None and existing is not msg_cls:
                        self.get_logger().error(
                            f"Topic {rt_name} already has publisher with type {existing.__name__}; skipping incompatible mapping from {mqtt_topic} ({msg_cls.__name__})"
            ...
            if sdk_methods:
                ...
                self._mqtt2ros_sdk_method[mqtt_topic] = sdk_methods[0] if len(sdk_methods) == 1 else sdk_methods
            ...
            for tool_name, config in self._io_config.items():
                if config.get('enabled') and 'service_name' in config:
                    ...
                    if srv_type_str == "ur_msgs/srv/SetIO" and SetIO is not None:
                        ...
                        self.get_logger().info(f"Initialized IO client for {tool_name} on {srv_name}")
        
        # Subscribe to joint updates for the specific twin_uuid
        if self._mapping is not None and hasattr(self._mapping, 'twin_uuid') and self._mapping.twin_uuid:
            twin_uuid = self._mapping.twin_uuid
            ...
            if self._io_config.get('tool0', {}).get('enabled'):
                self._mqtt_callbacks.setdefault(joint_update_topic, []).append(self._on_tool_control_update)
                self.get_logger().info(f"Registered tool control callback for {joint_update_topic}")
        else:
        ...
        try:
            if self._mqtt_adapter is None:
                ...
                self.get_logger().info("=" * 60)
                self.get_logger().info("Establishing MQTT Connection: paho-mqtt (FALLBACK)")
                self.get_logger().info("=" * 60)
                ...
                self.get_logger().info("=" * 60)
                self.get_logger().info("Establishing MQTT Connection: Cyberwave SDK (PRIMARY)")
                self.get_logger().info("=" * 60)
    ...
    def _get_virtual_joints(self) -> typing.Set[str]:
        ...
        if not hasattr(self, '_io_config') or not self._io_config:
            return set()
        return {config.get('joint_name') for config in self._io_config.values() if config.get('joint_name')}
    ...
    def _wait_for_adapter_connection(self, adapter, timeout: float = 5.0) -> None:
        ...
        self.get_logger().info(f"Waiting for Cyberwave SDK connection (timeout: {timeout}s)...")
        poll_interval = 0.1
        waited = 0.0
        ...
        if getattr(adapter, 'connected', False):
            ...
            self.get_logger().warning(f"Connection not confirmed after {timeout}s (may still be connecting)")

    def _get_or_create_publisher(self, topic: str, msg_cls: typing.Any) -> typing.Optional[typing.Any]:
    ...
    def _create_joint_command_callback(self, twin_uuid: str, cmd_pub, cmd_topic: str, cmd_msg_cls: typing.Any) -> typing.Callable[[typing.Any], None]:
        ...
        def _on_update(fake_msg):
            ...
            if jname_mqtt is not None and not isinstance(jname_mqtt, str):
                try:
                    ...
                    self.get_logger().debug(f"Normalized joint_name to string: {jname_mqtt}")
                except Exception:
                    pass
            ...
            val = next((_first_numeric(jstate.get(k)) for k in ('position', 'velocity', 'effort')), None)
            if val is None:
                return
            ...
            if mapping is not None and getattr(mapping, 'joint_names', None):
                try:
                    ...
                    if not isinstance(arr, list) or len(arr) != n:
                        ...
                        initial_state = self._accumulated_joint_states.get('initial_joint_state')
                    ...
                    try:
                        ...
                        rev = mapping.reverse_transforms.get(ros_name_for_idx, lambda x: x)
                        transformed = rev(val)
                    ...
                    if msg_type is JointTrajectory:
                        ...
                        joint_names_filtered = [jn for jn in mapping.joint_names if jn not in virtual_joints]
                        positions_filtered = [arr[i] for i, jn in enumerate(mapping.joint_names) if jn not in virtual_joints]
                        
                        ros_msg.joint_names = joint_names_filtered
                        # Calculate safe trajectory time based on distance and velocity limits
                        trajectory_time = self._calculate_trajectory_time(positions_filtered, joint_names_filtered)
                        ...
                        ros_msg.header.frame_id = str(source_type) if source_type else ""
                    ...
                    self.get_logger().error(f"Failed to build joint array for twin {twin_uuid}: {e}")
                    return
            else:
            ...
            if cmd_pub:
                try:
                    ...
                    self.get_logger().error(f"Failed to publish twin {twin_uuid} update to {cmd_topic}: {e}")
            else:
                self.get_logger().warning(f"No publisher available to forward twin {twin_uuid} update to {cmd_topic}")
    ...
    def _subscribe_twin_joint_states(self, method, twin_uuid: str, mqtt_topic: str) -> bool:
        ...
        pubs = self._mqtt2ros_pubs.get(mqtt_topic, [])
        if not pubs:
            self.get_logger().warning(f"No publishers configured for {mqtt_topic}; cannot use subscribe_twin_joint_states")
            return False
        ...
        def adapter_on_update(fake_msg):
            ...
            for cb in callbacks:
                try:
                    ...
                    self.get_logger().error(f"Adapter forwarding callback failed for {mqtt_topic}: {e}")
    ...
    def _load_mapping(self) -> None:
        ...
        if not os.path.isabs(path):
            path = os.path.join(mappings_dir, path) if not path.startswith(mappings_dir) else path
        ...
        try:
            ...
            if not self._mapping.twin_uuid:
                ...
                if env_uuid:
                    ...
                    self.get_logger().info(f"twin_uuid set from CYBERWAVE_TWIN_UUID env var: {env_uuid}")

            try:
                require_uuid = bool(self.get_parameter('mapping_require_digital_twin').value)
            except Exception:
                require_uuid = False
    ...
    def _on_start_video(self, request, response) -> typing.Any:
        ...
        try:
            ...
            self.get_logger().error(f"Failed to handle start camera stream service call: {e}")
            response.success = False
    ...
    def _on_stop_video(self, request, response) -> typing.Any:
        ...
        try:
            ...
            self.get_logger().error(f"Failed to handle stop camera stream service call: {e}")
            response.success = False
    ...
    def _make_ros_cb(self, ros_topic: str, mqtt_topic: str, msg_cls: typing.Any) -> typing.Callable[[typing.Any], None]:
        ...
        def cb(msg):
            try:
                ...
                if (msg_cls is BatteryState or msg_cls.__name__ == 'BatteryState'):
                    ...
                    self.get_logger().debug(f"Cached battery message: voltage={msg.voltage}V")
                elif (msg_cls is Float32 or msg_cls.__name__ == 'Float32') and 'battery' in mqtt_topic:
                    self._last_battery_msg = msg
                    self.get_logger().debug(f"Cached battery message from Float32: {msg.data}V")

                # Rate limiting
                interval = self._ros2mqtt_custom_intervals.get(ros_topic, self._ros2mqtt_rate_interval)
                if interval > 0:
                ...
                sdk_method = self._ros2mqtt_sdk_method.get(ros_topic) if hasattr(self, '_ros2mqtt_sdk_method') else None
                mapping = getattr(self, '_mapping', None)

                # Upstream filtering based on mapping mode
                if mapping is not None:
                    topic_type = 'joint' if 'cyberwave/joint/' in mqtt_topic else 'pose' if 'cyberwave/pose/' in mqtt_topic else None
                    if topic_type and not mapping.should_publish_topic(topic_type):
                        return

                is_odom = (msg_cls is Odometry or msg_cls.__name__ == 'Odometry' or 'Odometry' in str(msg_cls))
                
                # Fallback to internal odom if ROS /odom not available and internal odom is enabled
                if 'cyberwave/pose/' in mqtt_topic and '/update' in mqtt_topic and payload is None and \
                   mapping is not None and self._internal_odom:
                    try:
                ...
                if is_odom and ('position' in mqtt_topic or 'rotation' in mqtt_topic or 'update' in mqtt_topic):
                    try:
                        raw_pos = msg.pose.pose.position
                        raw_quat = msg.pose.pose.orientation
                        if 'update' in mqtt_topic:
                            payload_obj = {
                                "source_type": SOURCE_TYPE_EDGE, "type": "update",
                                "position": {"x": raw_pos.x, "y": raw_pos.y, "z": raw_pos.z},
                                "rotation": {"w": raw_quat.w, "x": raw_quat.x, "y": raw_quat.y, "z": raw_quat.z},
                                "ts": time.time()
                            }
                        ...
                        payload = self._encode_msg_for_mqtt(msg, msg_cls, mqtt_topic=mqtt_topic)
                
                elif (msg_cls is JointState or msg_cls.__name__ == 'JointState') and adapter is not None and sdk_method == 'update_joint_state' and getattr(self, '_mapping', None) is not None and getattr(self._mapping, 'twin_uuid', None):
                    ...
                    try:
                        ...
                        for ros_name, mqtt_name in mapping.name_to_mqtt.items():
                            ...
                            pos = float(msg.position[idx]) if msg.position and idx < len(msg.position) else None
                            vel = float(msg.velocity[idx]) if msg.velocity and idx < len(msg.velocity) else None
                            eff = float(msg.effort[idx]) if msg.effort and idx < len(msg.effort) else None
                            adapter.update_joint_state(twin, mqtt_name, position=pos, velocity=vel, effort=eff)
                        return
                    except Exception:
                        self.get_logger().debug(f"SDK update_joint_state failed for {ros_topic}")
                ...
                elif (msg_cls is Odometry or msg_cls.__name__ == 'Odometry') and adapter is not None and sdk_method == 'update_twin_pose' and getattr(self, '_mapping', None) is not None and getattr(self._mapping, 'twin_uuid', None):
                    try:
                        ...
                        self.get_logger().warning(f"SDK publish_position failed for {ros_topic}: {e}", exc_info=True)
                ...
                self.get_logger().error(f"Failed to publish to MQTT topic {mqtt_topic}: {e}")

        return cb

    def _encode_msg_for_mqtt(self, msg, msg_cls, mqtt_topic: str = None) -> typing.Union[str, bytes]:
        # Convert ROS message to an MQTT payload (bytes or string)
        try:
            # Special case for battery status when using Float32
            if (msg_cls is Float32 or msg_cls.__name__ == 'Float32') and mqtt_topic and 'battery' in mqtt_topic:
                try:
            ...
            mapping = getattr(self, '_mapping', None)
            if (msg_cls is JointState or msg_cls.__name__ == 'JointState') and mapping is not None:
                if getattr(self, '_disable_edge_joint_updates', False):
            ...
            try:
                ...
                if 'timestamp' not in payload_dict and hasattr(msg, 'header') and hasattr(msg.header, 'stamp'):
                    payload_dict['ts'] = float(msg.header.stamp.sec) + float(msg.header.stamp.nanosec) * 1e-9
    ...
    def _calculate_trajectory_time(self, target_positions: typing.List[float], joint_names: typing.List[str]) -> float:
        """
        ...
        current_positions = {}
        if hasattr(self, '_accumulated_joint_states') and 'initial_joint_state' in self._accumulated_joint_states:
        ...
        for idx, joint_name in enumerate(joint_names):
            ...
            max_accel = max_accelerations.get(joint_name, max_accelerations.get('default', 3.0))
            ...
            time_by_accel = 2.0 * math.sqrt(distance / max_accel) if max_accel > 0 else min_time
    ...
    def _create_smooth_trajectory_points(self, start_positions: typing.List[float], 
                                         target_positions: typing.List[float],
                                         total_time: float, num_points: int = 3) -> typing.List[JointTrajectoryPoint]:
        """
        ...
        for i in range(num_points):
            ...
            for j in range(len(target_positions)):
                start_pos = start_positions[j] if j < len(start_positions) else target_positions[j]
    ...
    def _republish_position_command(self) -> None:
        ...
        if self._last_position_command is not None and self._position_command_publisher is not None:
            try:
                # Update timestamp for trajectory messages
                if hasattr(self._last_position_command, 'header'):
                    self._last_position_command.header.stamp = self.get_clock().now().to_msg()
    ...
    def _handle_mqtt_connect(self, rc: int) -> None:
        if rc == 0:
            self.get_logger().info(f"Connected to MQTT broker {getattr(self,'_mqtt_host','?')}:{getattr(self,'_mqtt_port','?')}")
            ...
            if not hasattr(self, '_battery_update_timer') or self._battery_update_timer is None:
                self._battery_update_timer = self.create_timer(60.0, self._publish_battery_status)
                self.get_logger().info("Started periodic battery status updates (60s interval)")
        else:
    ...
    def _publish_battery_status(self) -> None:
        ...
        try:
            ...
            if self._last_battery_msg is None:
                self.get_logger().debug("No battery data available yet for periodic update")
                return
            
            # Get twin UUID
            if not hasattr(self, '_mapping') or self._mapping is None:
                self.get_logger().debug("No mapping available for battery status publishing")
                return
            
            twin_uuid = getattr(self._mapping, 'twin_uuid', None)
            if not twin_uuid:
                self.get_logger().debug("No twin_uuid available for battery status publishing")
                return
            ...
            payload_str = self._encode_msg_for_mqtt(self._last_battery_msg, type(self._last_battery_msg), mqtt_topic=topic)
            ...
            if self._mqtt_adapter:
                ...
                try:
                    ...
                    self.get_logger().info(f"Published periodic battery status to {topic}: voltage={payload_dict.get('voltage', 'N/A')}V, percentage={payload_dict.get('percentage', 'N/A')}")
                except Exception as e:
    ...
    def _handle_mqtt_message(self, topic: str, payload=None, mqtt_msg=None) -> None:
        ...
        if source_type == SOURCE_TYPE_EDGE and not is_signaling and topic not in self._mqtt_callbacks:
            return
        ...
        if topic.endswith('/command') and isinstance(data, dict):
            ...
            command_data = raw_command_data if isinstance(raw_command_data, dict) else {}
            ...
            if command in ["start_video", "stop_video", "camera_servo"]:
                if source_type != SOURCE_TYPE_TELE:
                    self.get_logger().warning(f"Ignoring {command} from non-tele source: {source_type} (expected: {SOURCE_TYPE_TELE})")
                    return
                else:
                    self.get_logger().info(f"Accepted {command} command from {source_type}")
            ...
            if command and self._command_registry is not None:
                try:
                    ...
                    actuation_commands = [
                        ...
                        'camera_up', 'camera_down', 'camera_left', 'camera_right', 'camera_default',
                        'chassis_light_toggle', 'camera_light_toggle', 'led_toggle',
                        'take_photo', 'battery_check', 'sit_down', 'stand_up', 
                        'obstacle_avoidance_toggle', 'start_video', 'stop_video'
                    ]
                    ...
                    if command in actuation_commands:
                        ...
                        success = self._command_registry.handle_command('actuation', data)
                    else:
                        # Other commands use the split format (command name + data only)
                        success = self._command_registry.handle_command(command, command_data)
            ...
            for pattern in matched_patterns:
                ...
                for pub, msg_cls, ros_topic in entries:
                    try:
                        ...
                        if (msg_cls is JointState or msg_cls is JointTrajectory or '/joint_states' in ros_topic):
                            if source_type != SOURCE_TYPE_TELE:
                                self.get_logger().info(
                        ...
                        if msg_cls is JointTrajectory and isinstance(ros_msg, JointTrajectory):
                            if not ros_msg.joint_names or not ros_msg.points:
                                ...
                                continue
                            if ros_msg.points and len(ros_msg.points[0].positions) != len(ros_msg.joint_names):
                                self.get_logger().error(
                            ...
                            if ros_msg.points:
                                ...
                                last_point_time = float(last_point.time_from_start.sec) + float(last_point.time_from_start.nanosec) * 1e-9
                            
                            self.get_logger().info(f"Trajectory: {len(ros_msg.joint_names)} joints, {len(ros_msg.points)} points, {last_point_time:.2f}s")
                        
                        pub.publish(ros_msg)
                    except Exception as e:
                        self.get_logger().error(f"Failed to publish ROS message for {topic} -> {ros_topic}: {e}")
                ...
                if self._match_mqtt_pattern(pattern, topic):
                    for h in wildcard_handlers:
                        try:
                            ...
                            self.get_logger().error(f"Error in MQTT wildcard callback for {pattern}: {e}")
    ...
    def _decode_payload_to_msg(self, payload: str, msg_cls) -> typing.Any:
        ...
        if msg_cls is JointState:
            ...
            if isinstance(data, dict) and data.get('type') == 'joint_state' and 'joint_name' in data and 'joint_state' in data:
                try:
                    ...
                    if mapping is not None:
                        ...
                        n = len(js.name)
                        js.position, js.velocity, js.effort = [float('nan')] * n, [float('nan')] * n, [float('nan')] * n
                        try: idx = js.name.index(ros_name)
                        except ValueError: idx = None
                        rev = mapping.reverse_transforms.get(ros_name, lambda x: x)
                        if idx is not None:
                            if 'position' in jstate: js.position[idx] = float(rev(jstate['position']))
                            if 'velocity' in jstate: js.velocity[idx] = float(rev(jstate['velocity']))
                            if 'effort' in jstate: js.effort[idx] = float(rev(jstate['effort']))
                        return js
                    else:
                        js = JointState(name=[jname_mqtt])
                        js.header.stamp = self.get_clock().now().to_msg()
                        pos = jstate.get('position')
                        js.position = [float(pos)] if pos is not None else [float('nan')]
                        return js
                except Exception: pass

            if mapping is not None:
                try: return mapping.remap_mqtt_to_ros(payload)
                except Exception: pass
        if msg_cls is Int32: return Int32(data=int(payload))
        if msg_cls is Float32: return Float32(data=float(payload))
        if msg_cls is UInt32MultiArray:
            m = UInt32MultiArray()
            try: m.data = [int(x) for x in json.loads(payload)]
            except Exception: m.data = []
            return m
        if msg_cls is Float32MultiArray:
            m = Float32MultiArray()
            try: m.data = [float(x) for x in json.loads(payload)]
            except Exception: m.data = []
            return m
        if msg_cls is Float64MultiArray:
            ...
            try:
                ...
                if isinstance(data, dict) and mapping is not None:
                    m.data = [float(data.get(mapping.name_to_mqtt.get(n, n), 0.0)) for n in mapping.joint_names]
                elif isinstance(data, list): m.data = [float(x) for x in data]
            except Exception: m.data = []
            return m
        if msg_cls is JointTrajectory:
            ...
            try:
                data = json.loads(payload) if isinstance(payload, str) else payload
                if isinstance(data, dict) and 'joint_name' in data and 'joint_state' in data:
                    if mapping is not None:
                        jname_mqtt, jstate = data.get('joint_name'), data.get('joint_state', {})
                        ...
                        if jname_mqtt is not None and pos_val is not None:
                            ...
                            if not hasattr(self, '_accumulated_joint_states'): self._accumulated_joint_states = {}
                            if state_key not in self._accumulated_joint_states:
                                self._accumulated_joint_states[state_key] = [0.0] * len(mapping.joint_names)
                            try:
                                idx = list(mapping.joint_names).index(ros_name)
                                self._accumulated_joint_states[state_key][idx] = float(mapping.reverse_transforms.get(ros_name, lambda x: x)(pos_val))
                            except Exception: pass
                        ...
                        for jn in joint_names_filtered:
                            ...
                            start_positions.append(initial_state[idx] if initial_state and idx < len(initial_state) else target_val)
                        m.joint_names = joint_names_filtered
                        traj_time = self._calculate_trajectory_time(positions_filtered, joint_names_filtered)
                        m.points = self._create_smooth_trajectory_points(start_positions, positions_filtered, traj_time, num_points=3)
                        m.header.stamp = self.get_clock().now().to_msg()
                        return m
                if isinstance(data, dict) and mapping is not None:
                    virtual_joints = self._get_virtual_joints()
                    ...
                    for jn in joint_names_filtered:
                        ...
                        val = float(mapping.reverse_transforms.get(jn, lambda x: x)(data[mqtt_n])) if mqtt_n in data else (initial_state[idx] if initial_state and idx < len(initial_state) else 0.0)
                        positions.append(val); start_positions.append(initial_state[idx] if initial_state and idx < len(initial_state) else val)
                    if not self._joint_state_initialized:
                        m.header.stamp = self.get_clock().now().to_msg()
                        return m
                    m.joint_names = joint_names_filtered
                    traj_time = self._calculate_trajectory_time(positions, joint_names_filtered)
                    m.points = self._create_smooth_trajectory_points(start_positions, positions, traj_time, num_points=3)
                    m.header.stamp = self.get_clock().now().to_msg()
                    return m
                if isinstance(data, list):
                    if mapping is not None:
                        virtual_joints = self._get_virtual_joints()
                        m.joint_names = [jn for jn in mapping.joint_names if jn not in virtual_joints]
                        positions = [float(x) for x in data[:len(m.joint_names)]]
                    else: positions = [float(x) for x in data]
                    traj_time = self._calculate_trajectory_time(positions, m.joint_names if mapping else [])
                    p = JointTrajectoryPoint(positions=positions, velocities=[], accelerations=[])
                    p.time_from_start.sec = int(traj_time); p.time_from_start.nanosec = int((traj_time - int(traj_time)) * 1e9)
                    m.points = [p]; m.header.stamp = self.get_clock().now().to_msg()
                    return m
            except Exception: pass
            return m
        return String(data=payload)
    ...
    def publish(self, topic: str, message, qos: int = 0) -> typing.Any:
        ...
        if 'webrtc' in final_topic or 'command' in final_topic:
            self.get_logger().info(f"MQTT OUTGOING: topic={final_topic}, content={str(message)[:100]}...")
        ...
        try:
            ...
            self.get_logger().error(f"Failed to publish to MQTT topic {final_topic}: {e}")
            raise

    def subscribe(self, topic: str, on_message: typing.Callable = None, qos: int = 0) -> None:
        """SDK-compatible subscribe method.
        
        Registers a callback and ensures the underlying MQTT client is subscribed.
        """
        self.get_logger().info(f"Subscribed to topic: {topic} (on_message: {on_message is not None})")
        if on_message:
    ...
    def subscribe_pong(self, resource_uuid: str, on_pong: typing.Optional[typing.Callable] = None) -> None:
        ...
        try:
            ...
            for tool_name, config in self._io_config.items():
                ...
                if self._robot_io_client.wait_for_service(timeout_sec=1.0):
                    ...
                    self.get_logger().info(f"Tool '{tool_name}': {state_str} (pos={position_value:.3f})")
                    
                    future = self._robot_io_client.call_async(request)
                    future.add_done_callback(lambda f, tn=tool_name, s=state, p=position_value: 
                                             self._tool_io_response_callback(f, tn, s, p))
            
        except Exception as e:
            self.get_logger().error(f"Failed to process tool update: {e}")
    
    def _tool_io_response_callback(self, future, tool_name: str, requested_state: float, position: float) -> None:
        ...
        try:
            ...
            if response.success:
                ...
                if self._mapping and self._mapping.twin_uuid:
                    ...
                    topic = f"{prefix}cyberwave/twin/{self._mapping.twin_uuid}/status/io"
                    self._mqtt_client.publish(topic, json.dumps(status_payload))
            else:
                self.get_logger().error(f"IO service call failed for tool '{tool_name}'")
        except Exception as e:
    ...
    def _maybe_auto_start_webrtc(self) -> None:
        ...
        if not getattr(self._mqtt_adapter, "connected", False):
            self.get_logger().warning("WebRTC auto-start skipped: MQTT adapter not connected yet")
            self._schedule_auto_start_retry()
            return

        twin_uuid = getattr(self._mapping, 'twin_uuid', None)
        if not twin_uuid:
            self.get_logger().warning("WebRTC auto-start skipped: twin_uuid missing in mapping")
            self._schedule_auto_start_retry()
            return
    ...
    def _start_webrtc_with_auto_reconnect(self) -> None:
        ...
        async def _run_with_auto_reconnect():
            try:
                ...
                self.get_logger().info("Waiting for camera frames before starting WebRTC...")
        ...
        future = asyncio.run_coroutine_threadsafe(_run_with_auto_reconnect(), self._async_loop)
        
        # Store the future so we can check its status later
        self._webrtc_auto_reconnect_future = future
        
        self.get_logger().info("WebRTC auto-reconnect loop started (waiting for start_video commands)")
    ...
    def start_camera_stream(self, recording: bool = True, fps: Optional[int] = None) -> None:
        ...
        try:
            twin_uuid = getattr(self._mapping, 'twin_uuid', None)
            if not twin_uuid:
                self.get_logger().error("Cannot start camera stream: twin_uuid not found in mapping")
                return
            ...
            if hasattr(self, '_webrtc_start_future') and self._webrtc_start_future is not None:
                if not self._webrtc_start_future.done():
                    self.get_logger().warning("WebRTC start already in progress; ignoring duplicate start_video command")
                    return
            
            existing_pc = getattr(self._ros_streamer, "pc", None)
            if existing_pc and existing_pc.connectionState not in ["closed", "failed"]:
                self.get_logger().info("WebRTC streamer already active; skipping restart")
                return
        ...
        if hasattr(self, '_ros_streamer') and self._ros_streamer is not None:
            try:
                ...
                if twin_uuid:
                    ...
                    self.publish(topic, payload)
                    self.get_logger().info(f"Sent video_stopped notification to {topic}")
            except Exception as e:
    ...
    def _check_camera_status(self) -> None:
        ...
        if hasattr(self, '_mapping') and self._mapping:
            video_device = self._mapping.raw.get('camera', {}).get('video_device', video_device)
        
        device_exists = os.path.exists(video_device)
        
        # 2. Check if we are receiving images
        is_receiving = (now - self._last_image_time) < 5.0 # Receiving if last image was < 5s ago
        ...
        if not device_exists:
            self.get_logger().error(f"CAMERA WATCHDOG: Device {video_device} NOT FOUND! Check connection.")
        elif not usb_cam_running:
            self.get_logger().error(f"CAMERA WATCHDOG: usb_cam node is NOT RUNNING! Attempting to restart via WebRTC trigger...")
        elif not is_receiving:
            # Check if we have a streamer and it's actually subscribed
            streamer_active = False
            if hasattr(self, '_ros_streamer') and self._ros_streamer and self._ros_streamer.streamer:
                streamer_active = True
            
            if streamer_active:
                self.get_logger().warn(f"CAMERA WATCHDOG: Device {video_device} exists but NO IMAGES on /image_raw! Is the camera busy?")
                if now - self._last_image_time > 10.0: # 10 seconds of silence while streaming
                    self.get_logger().error("CAMERA WATCHDOG: Streamer is active but no data for 10s. Forcing reconnection.")
            else:
                ...
                self.get_logger().debug("CAMERA WATCHDOG: No active streamer, checking watchdog subscription...")
        else:
            # Everything seems fine, log periodically (every 60s instead of 30s to avoid spam)
            if now - self._last_camera_check_time > 60.0:
                self.get_logger().info(f"CAMERA WATCHDOG: Camera OK ({video_device} active, streaming at /image_raw)")
                # Also log current image stats if available
                if hasattr(self, '_ros_streamer') and self._ros_streamer and self._ros_streamer.streamer:
                    track = self._ros_streamer.streamer
                    self.get_logger().info(f"CAMERA STATS: {track.actual_width}x{track.actual_height} @ {track.fps}fps, encoding={track.encoding}")
    ...
    def destroy_node(self) -> None:
        try:
            if hasattr(self, '_ros_streamer') and self._ros_streamer: 
                ...
                asyncio.run_coroutine_threadsafe(self._ros_streamer.stop(), self._async_loop)
        ...
        try:
            ...
            else: self._mqtt_client.loop_stop(); self._mqtt_client.disconnect()
        except Exception: pass
        super().destroy_node()
...
def main(args=None) -> None:
    ...
    try: rclpy.spin(node)
        ...
        import traceback
        logging.getLogger('mqtt_bridge_node').error(f"Node crashed: {e}\n{traceback.format_exc()}")
    finally:
