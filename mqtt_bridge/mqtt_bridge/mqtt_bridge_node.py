...
class MQTTBridgeNode(Node):
    def __init__(self) -> None:
        ...
        p_token = (
            self.get_parameter("broker.cyberwave_token").value
            or self.get_parameter("cyberwave.api_token").value
        )
        ...
        token = p_token or os.getenv("CYBERWAVE_API_KEY")
        host = p_host or mqtt_broker_env
        env = p_env or os.getenv("CYBERWAVE_ENVIRONMENT")
        uuid = os.getenv("CYBERWAVE_EDGE_UUID")

        self.get_logger().info(f"CYBERWAVE_API_KEY: {token if token else 'NOT SET'}")
        ...
        if use_cw and CyberwaveAdapter is not None:
            ...
            if token:
                ...
                self.get_logger().error(
                    "Cyberwave API Token not found! 'use_cyberwave' is enabled but no token was provided via parameters or environment (CYBERWAVE_API_KEY). Video streaming will be unavailable."
                )
    ...
    def _make_ros_cb(
        ...
        def cb(msg):
            try:
                ...
                ) and "/status/battery" in mqtt_topic:
                    self._last_battery_msg = msg
