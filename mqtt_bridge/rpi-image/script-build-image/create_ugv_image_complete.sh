...
#   - All system configurations (udev, docker, audio, etc.)
#   - User environment (.bashrc, helper scripts)
#   - Python packages (from .local)
#
# Base Image: Raspberry Pi OS Lite (Debian Trixie/13) ARM64
#
# Requirements:
#   - Linux host with root privileges
#   - Pre-built ROS 2 Jazzy on host (from ros-realtime-rpi4-image-builder.sh)
#   - ~15GB free disk space
#   - Tools: wget, xz, kpartx, parted, rsync
#
# Usage:
#   sudo ./create_ugv_image_complete.sh
#
################################################################################
...
HOST_USER_WS="${ACTUAL_HOME}/ws"             # User workspace directory
...
# Base image - Raspberry Pi OS Lite 64-bit (Debian Trixie based)
# Official Raspberry Pi OS - supports Pi 5 natively
# Latest release: December 2025, kernel 6.12, Debian 13 (Trixie)
BASE_IMAGE_URL="https://downloads.raspberrypi.com/raspios_lite_arm64/images/raspios_lite_arm64-2025-12-04/2025-12-04-raspios-trixie-arm64-lite.img.xz"
BASE_IMAGE_NAME="raspios-trixie-arm64-lite.img"
OUTPUT_IMAGE_NAME="CYB_UGV_Beast_ROS2_Jazzy_Trixie_$(date +%Y%m%d).img"
...
log_info "Raspberry Pi OS (Debian Trixie) + ROS 2 Jazzy"
log_info "========================================"
echo ""
...
if [ ! -d "$HOST_ROS2_INSTALL" ]; then
    ...
    log_info "Please run ros-realtime-rpi4-image-builder.sh first to build ROS 2."
    exit 1
fi
...
if [ ! -d "$HOST_UGV_WS" ]; then
    ...
    log_info "Please run ros-realtime-rpi4-image-builder.sh first to build the workspace."
    exit 1
fi
...
TOTAL_COPY_SIZE=$((ROS2_SIZE + UGV_SIZE + LOCAL_SIZE + ROS2_SRC_SIZE))

log_info "Space breakdown:"
log_info "  ROS 2 Jazzy install: ${ROS2_SIZE}MB"
log_info "  UGV workspace: ${UGV_SIZE}MB"
...
log_step "[1/8] Setting up directories..."
...
log_step "[2/8] Preparing Raspberry Pi OS (Trixie) base image..."
...
if [ ! -f "$BASE_IMAGE_PATH" ]; then
    if [ ! -f "$BASE_IMAGE_XZ" ]; then
        log_info "Downloading Raspberry Pi OS Lite ARM64 (Debian Trixie based)..."
...
log_step "[3/8] Resizing image to fit ROS 2 + workspace..."
...
log_step "[5/8] Copying ROS 2 Jazzy installation..."
...
log_step "[6/8] Copying user workspace and configuration..."
...
log_step "[7/8] Configuring system..."
...
# Configure user .bashrc with ROS 2 environment
...
# Create ALSA audio configuration
...
# Configure boot settings for ROS 2 realtime performance
...
################################################################################
# Stage 8: Create First Boot Script
################################################################################

log_step "[8/8] Creating first boot configuration script..."
...
echo ""
echo "[6/7] Disabling unnecessary services for ROS..."
...
echo ""
echo "[7/7] Updating library cache..."
sudo ldconfig
...
log_info "========================================"
log_info "  Base OS: Raspberry Pi OS Lite (Debian Trixie)"
